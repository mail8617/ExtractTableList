...
EXEC PR_CHK_DROP_TABLE('FL_MK_MIDPROMO_CHNG_TEMP');

CREATE TABLE FL_MK_MIDPROMO_CHNG_TEMP NOLOGGING AS
SELECT /*+ PARALLEL(4) */
       DISTINCT T2.MDPRCD
     , T1.STR_CD
     , T1.INTG_MEMB_NO
...
EXEC PR_CHK_DROP_TABLE('TEMP_MIDPROMO_ACTRSLT_01');

/* 교환권관련 */
CREATE TABLE TEMP_MIDPROMO_ACTRSLT_01 NOLOGGING AS
SELECT /*+ PARALLEL(4) */
       T1.MDPRCD
     , T1.STR_CD
     , T1.STD_DT
     , T1.INTG_MEMB_NO                 AS INTG_MEMB_NO
     , T1.INTG_CUST_DISTING_NO
     , T1.CUST_DISTING_NO              AS CUST_DISTING_NO
     , T1.CUST_SALES_DVS_CD            AS CUST_SALES_DVS_CD
     , T1.SL_CHNL_CD
     , T1.CUST_CLSF_CD
     , T1.GRP_CLSF_CD
     ...
     (
       SELECT /*+ USE_HASH(T2) PARALLEL(4) */
           ...
           -- , T1.WON_ARSE_NSALAMT
            , T1.IMGN_INFO_DVS_CD       /* 출입국정보구분코드*/
            , T1.IMGN_DT                /* 출입국일자 */
            , ROW_NUMBER() OVER (PARTITION BY T1.MDPRCD, T1.STR_CD, T1.STD_DT, T1.EXCH_NO ORDER BY T1.EXCH_PRMTN_APLY_YN DESC, T1.PRMTNCD) AS RN
         FROM FL_MK_PRMTN_EXCH_SL T1             /* FL_MK_행사교환권판매 */
        INNER JOIN FL_MK_MIDPROMO_CHNG_TEMP T2  /* 변경적재 대상 */
  ...
  LEFT OUTER JOIN (
       SELECT /*+ USE_HASH(T2) PARALLEL(4) */
            ...
            , T1.DLR_ARSE_NSALAMT
            , T1.WON_ARSE_NSALAMT
            , ROW_NUMBER() OVER (PARTITION BY T1.MDPRCD, T1.STR_CD, T1.STD_DT, T1.EXCH_NO ORDER BY T1.PRMTNCD) AS RNUM
         FROM FL_MK_PRMTN_EXCH_SL T1            /* FL_MK_행사교환권판매 */
        INNER JOIN FL_MK_MIDPROMO_CHNG_TEMP T2  /* 변경적재 대상 */
           ON T1.MDPRCD       = T2.MDPRCD
          AND T1.STR_CD       = T2.STR_CD
...
DELETE /*+ PARALLEL(4) */
  FROM FL_MK_MIDPROMO_ACTRSLT T1
 WHERE EXISTS (
               SELECT 1
                 FROM FL_MK_MIDPROMO_CHNG_TEMP T0
...
FROM
     (
        SELECT /*+ PARALLEL(4) */
             ...
             , MAX(IMGN_INFO_DVS_CD       )      AS IMGN_INFO_DVS_CD
             , MAX(IMGN_DT                )      AS IMGN_DT
          ...
          FROM ( /* 교환권 관련 매출 */
              SELECT /*+ PARALLEL(4) */
                ...
                FROM TEMP_MIDPROMO_ACTRSLT_01 T1
               UNION ALL
              /* 할인금액 */
              SELECT /*+ PARALLEL(4) */
                     T2.MDPRCD
                   ...
                   , 0                               AS DLR_CROS_ARSE_NSALAMT   /* 달러교차유발순매출액     */
                   , 0                               AS WON_CROS_ARSE_NSALAMT   /* 원화교차유발순매출액     */
                   , 0                               AS OMNI_DLR_NSALAMT        /* 옴니달러순매출액         */
                   , 0                               AS OMNI_WON_NSALAMT        /* 옴니원화순매출액         */
               ...
               INNER JOIN FL_MK_MIDPROMO_CHNG_TEMP T3  /* 변경적재 대상 */
 ...
 INNER JOIN D_MIDPROMO T5  /* D_중행사 */
    ON T1.MDPRCD= T5.MDPRCD
  LEFT OUTER JOIN D_MEMB T2 /* D_회원 */
    ON T1.INTG_MEMB_NO = T2.INTG_MEMB_NO
  LEFT OUTER JOIN WL_SL_STR_INFO T3  /* WL_SL_점정보 */
    ON T1.STR_CD = T3.STR_CD
...
MERGE /*+ PARALLEL(4) */
 INTO FL_MK_MIDPROMO_ACTRSLT T1
USING (SELECT T2.MDPRCD
            ...
            , T2.DLR_EXCH_NSALAMT
            , T2.WON_EXCH_NSALAMT
         FROM FL_MK_MIDPROMO_ACTRSLT  T2
        INNER JOIN
            ( SELECT DISTINCT
                     T2.MDPRCD
                   , T3.STR_CD
