...
EXEC PR_CHK_DROP_TABLE('FE_MK_ORD_TEMP');

CREATE TABLE FE_MK_ORD_TEMP NOLOGGING AS
SELECT T1.ONLN_ORD_NO
      ,T0.MBR_NO AS ONLN_MEMB_NO
      ,T0.ORD_DT
FROM WE_OD_ORD T0
JOIN (
       SELECT /*+ PARALLEL(4) */
              T01.ORD_NO          AS ONLN_ORD_NO
         FROM WE_OD_ORD T01  /* WE_주문 */
        WHERE T01.SYS_MOD_DTIME BETWEEN TO_DATE('$$[DW_STRT_DT]','YYYYMMDD') AND TO_DATE('$$[DW_END_DT]','YYYYMMDD') + 0.99999
        UNION
       SELECT /*+ PARALLEL(4) */
              T01.ORD_NO
         FROM WE_OD_ORD_DSCNT T01 /* WE_주문할인 */
        WHERE T01.SYS_MOD_DTIME BETWEEN TO_DATE('$$[DW_STRT_DT]','YYYYMMDD') AND TO_DATE('$$[DW_END_DT]','YYYYMMDD') + 0.99999
        UNION
       SELECT /*+ PARALLEL(4) */
              T01.ORD_NO
         FROM WE_MB_SVMN_RSRV_USE T01 /* WE_적립금적립사용 */
        WHERE T01.SYS_MOD_DTIME BETWEEN TO_DATE('$$[DW_STRT_DT]','YYYYMMDD') AND TO_DATE('$$[DW_END_DT]','YYYYMMDD') + 0.99999
        UNION
       SELECT /*+ PARALLEL(4) */
              T01.ORD_NO
         FROM WE_MT_ISSU_CPN T01  /* WE_발행쿠폰 */
        WHERE T01.SYS_MOD_DTIME BETWEEN TO_DATE('$$[DW_STRT_DT]','YYYYMMDD') AND TO_DATE('$$[DW_END_DT]','YYYYMMDD') + 0.99999
        UNION
       SELECT /*+ PARALLEL(4) */
              T01.ORD_NO
         FROM WE_OD_CLAIM T01    /* WE_클레임 */
        WHERE T01.SYS_MOD_DTIME BETWEEN TO_DATE('$$[DW_STRT_DT]','YYYYMMDD') AND TO_DATE('$$[DW_END_DT]','YYYYMMDD') + 0.99999
        UNION
       SELECT /*+ PARALLEL(4) */
              T01.ONLN_ORD_NO
         FROM FL_MK_LDFP_ACMLT_USE_PTCLS T01    /* FL_MK_LDFPAY적립사용내역 */
        WHERE T01.LDFP_HIST_HAPN_DT BETWEEN '$$[DW_STRT_DT]' AND '$$[DW_END_DT]'
       ) T1 
   ON T0.ORD_NO = T1.ONLN_ORD_NO
;
...
EXECUTE IMMEDIATE 'TRUNCATE TABLE FE_MK_PRMTN_ORD_TEMP';
  ...
  FROM
     (
       ...
       SELECT /*+ USE_HASH(T0 T1) PARALLEL(4) */
            ...
            , NVL(T7.INTG_MEMB_NO, T6.INTG_MEMB_NO) AS INTG_MEMB_NO
        ...
        INNER JOIN WE_OD_ORD T5        /* WE_주문 */
         ...
         /* 변경적재 모수 ord_no조인 */
        INNER JOIN (SELECT DISTINCT ONLN_ORD_NO FROM FE_MK_ORD_TEMP) T0
           ON T4.ORD_NO = T0.ONLN_ORD_NO
         ---------------------------------------
         LEFT OUTER JOIN DE_ONLN_MEMB T6  /* DE_온라인회원 */
           ON T1.MBR_NO = T6.ONLN_MEMB_NO
         LEFT OUTER JOIN D_INTG_MEMB T7 ON T1.MBR_NO =  T7.ONLN_MEMB_NO 
       ...
       SELECT /*+ USE_HASH(T0 T1) PARALLEL(4) */
        ...
        INNER JOIN D_PRMTN T4   /* D_행사 */
         ...
         /*-- 변경적재 모수 ord_no조인----------------*/
        INNER JOIN (SELECT DISTINCT ONLN_ORD_NO FROM FE_MK_ORD_TEMP) T0
           ON T2.ORD_NO = T0.ONLN_ORD_NO
         ---------------------------------------
         LEFT OUTER JOIN DE_ONLN_MEMB T6  /* DE_온라인회원 */
           ON T2.MBR_NO            = T6.ONLN_MEMB_NO
         LEFT OUTER JOIN D_INTG_MEMB T7 ON T2.MBR_NO =  T7.ONLN_MEMB_NO 
       ...
       SELECT /*+ USE_HASH(T3 T0) PARALLEL(4) */
         ...
         FROM FL_MK_LDFP_ACMLT_USE_PTCLS  T0 /* FL_MK_LDFPAY적립사용내역 */
         /*-- 변경적재 모수 ord_no조인----------------*/
        INNER JOIN (SELECT DISTINCT ONLN_ORD_NO FROM FE_MK_ORD_TEMP) T3
...
DBMS_STATS.GATHER_TABLE_STATS('LDF_DW', 'FE_MK_PRMTN_ORD_TEMP', CASCADE=>TRUE, METHOD_OPT=>'FOR ALL INDEXED COLUMNS', DEGREE=>4);